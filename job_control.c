...
job * new_job(pid_t pid, const char * command,char *args[], enum job_state state, int tiempo)
{
	...
	if(args!=NULL){
		...
		//Debe acabar en NULL sino puede no funcionar
		aux->args[i]=NULL;
	}
...
// ------------------------------------------------- 
// FUNCIÓN IMPLEMENTADA POR PROFESOR 
/*Imprime mensaje de interpretacion */

void print_analyzed_status(enum status s, int info){
	switch (s){
		case SUSPENDED: 
		puts("\e[97m");
		printf("Proceso suspendido\n");
				break;
		case SIGNALED:  
		puts("\e[97m");
		printf("Proceso finalizado por la señal %i\n", info);
				break;
		case EXITED:    
		puts("\e[97m");
		printf("Proceso acabado con normalidad (retorno=%i)\n", info);
				break;
	}
} 
